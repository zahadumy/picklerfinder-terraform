...
#The Task Definition used in conjunction with the ECS service
resource "aws_ecs_task_definition" "ai-integration_td" {
    family                              = "ai-integration-td"
    # container definitions describes the configurations for the task
    container_definitions               = jsonencode(
    [
    {
        "name"                          : "ai-integration-container",
        "image"                         : "${aws_ecr_repository.ai-integration_ecr.repository_url}:latest",
        "entryPoint"                    : []
        "essential"                     : true,
        "networkMode"                   : "awsvpc",
        "portMappings"                  : [
...
resource "aws_ecs_service" "ai-integration_service" {
  name                                = "ai-integration-ecs-service"
  cluster                             = aws_ecs_cluster.development_ecs_cluster.arn
  task_definition                     = aws_ecs_task_definition.ai-integration_td.arn
  launch_type                         = "FARGATE"
  scheduling_strategy                 = "REPLICA"
  desired_count                       = 1 # the number of tasks you wish to run
  health_check_grace_period_seconds   = 180
